...
import type { VocabularyAnalysis, VocabularyAnalysisSearchRequest } from '@/shared/api/types';
...
function normalizeItem(
  ...
  const simplifiedDefinition = item.simplifiedDefinition ?? item.simplified_definition ?? '';
  const difficultyLevel = item.difficultyLevel ?? item.difficulty_level ?? 'medium';
  const gradeLevel = item.gradeLevel ?? item.grade_level ?? 0;
  const phonemeAnalysisJson = item.phonemeAnalysisJson ?? item.phoneme_analysis_json ?? '';
...
export const vocabularyAnalysisApi = {
  async search(params: VocabularyAnalysisSearchRequest): Promise<VocabularyAnalysis[]> {
