...
export default function ContentManagePage() {
  ...
  const lessons = [
    { id: 1, title: "우리 동네 동물들", grade: "초등 1-2학년", uploadDate: "2024-01-15", assigned: 2, status: "완료", pageCount: 20, progress: 100, color: "bg-orange-300" },
    { id: 2, title: "우주 탐험 이야기", grade: "초등 3-4학년", uploadDate: "2024-01-12", assigned: 1, status: "변환중", pageCount: 0, progress: 25, color: "bg-blue-700" },
    { id: 3, title: "마법의 숲 모험", grade: "초등 1-2학년", uploadDate: "2024-01-10", assigned: 0, status: "완료", pageCount: 18, progress: 100, color: "bg-green-400" },
  ];
          ...
          {filteredLessons.map((lesson) => (
            ...
            >
              <CardHeader className="pb-4">
                <div className={`${lesson.color} h-32 rounded-lg mb-4 flex items-center justify-center relative`}>
                  <div className="text-center text-white">
                    <FileText className="w-8 h-8 mx-auto mb-2 opacity-80" />
                    <p className="text-sm font-medium">{lesson.pageCount}페이지</p>
                  </div>
                  {lesson.status === "변환중" && (
                    <div className="absolute inset-0 bg-black/50 rounded-lg flex flex-col items-center justify-center">
                      <div className="text-white text-center w-full px-4">
                        ...
                        <div className="w-full">
                          <Progress value={lesson.progress} className="h-2 bg-white/20" />
                          <p className="text-xs mt-1">{Math.round(lesson.progress)}%</p>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <CardDescription className="mt-1">{lesson.grade}</CardDescription>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${lesson.status === "완료" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                <div className="flex space-x-2">
                  <Button
                    onClick={() => navigate({ to: ViewerRoute.id, params: { documentId: String(lesson.id) } })}
                    variant="outline"
                    className="flex-1"
                  ...
                  <Button variant="outline" className="flex-1" disabled={lesson.status !== "완료"}>
        ...
        {filteredLessons.length === 0 && (
          <Card className="border-dashed border-2 border-gray-300 mt-8">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-500 mb-4">"{searchTerm}"와 일치하는 교안을 찾을 수 없습니다</p>
              <Button className="bg-primary hover:bg-primary/90">
                <Upload className="w-4 h-4 mr-2" />
                교안 업로드하기
              </Button>
