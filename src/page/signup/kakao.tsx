...
export function KakaoAuthPage() {
  ...
  const handleKakaoAuth = () => {
    const redirectUri = `${window.location.origin}/signup/kakao?userType=${userType}`;
  ...
  useEffect(() => {
    ...
    const fetchKakaoTokenAndUserInfo = async () => {
      ...
      try {
        ...
        data.append('redirect_uri', `${window.location.origin}/signup/kakao?userType=${userType}`);
        ...
        if (!tokenData.access_token) {
          alert(`카카오 인증 실패: ${tokenData.error_description || JSON.stringify(tokenData)}`);
          return;
        }
